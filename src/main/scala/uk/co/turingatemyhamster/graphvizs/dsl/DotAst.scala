...
trait DotAstBuilder extends DotAst {
  ...
  def handle_nodeStatement(node: NodeId, attributes: Option[Seq[AttributeAssignment]]) =
    NodeStatement(node, attributes)

  def handle_edgeStatement(node: Node, nodes: Seq[(EdgeOp, Node)], attributes: Option[Seq[AttributeAssignment]]) =
    EdgeStatement(node, nodes, attributes)

  def handle_attributeStatement(statementType: StatementType, attributes1: Option[Seq[AttributeAssignment]], attributes2: Option[Seq[AttributeAssignment]]) =
    AttributeStatement(statementType, attributes1, attributes2)
